...
```{ojs}
//| echo: false
DOM.download(() => serialize(chart.children[1]), undefined, "Download SVG")
```
:::
...
The CSV files contains comments lines starting with # that we need to remove before parsing the data.
